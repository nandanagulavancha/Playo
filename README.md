...
5. To remove the containers, networks, and volumes created by Docker Compose, run the following command:
   ```bash
    ...
    ```
    - The `-v` flag is used to remove the associated volumes, which will delete any data stored in the databases. Use this command with caution if you want to preserve your data.
    - To remove the specific volumes without stopping the services, you can use:
      ```bash
      docker volume ls
